...
# Feature dimensions for each DepthAnythingV2 encoder variant.
# Dict-only lookup — no file I/O — so pretrain_dim can be derived even
# when the checkpoint is absent (i.e. when da_feature is not loaded).
_DAV2_DIM = {'vitl': 256, 'vitb': 128, 'vits': 64}
...
class ViTWarpV8(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.args = args
        # Load DepthAnything feature extractor only when the backbone type
        # explicitly requests it. For non-dav2/dinov3 configs (e.g. when
        # this module is instantiated for weight inspection without the
        # depth-anything checkpoint present), da_feature is left as None.
        if args.network_backbone in ('dav2', 'dinov3'):
            self.da_feature = self.freeze_(DepthAnythingFeature(encoder=args.dav2_backbone))
        else:
            self.da_feature = None
        # pretrain_dim from dict — safe whether or not da_feature is loaded.
        self.pretrain_dim = _DAV2_DIM[args.dav2_backbone]
        # The refinement ViT size mirrors the dav2_backbone variant used at
        # training time (both are 'vits' in the downstream checkpoint).
        self.network_dim = MODEL_CONFIGS[args.dav2_backbone]['features']
        self.refine_net = VisionTransformer(args.dav2_backbone, self.network_dim, patch_size=8)
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        ...
        if self.da_feature is None:
            raise RuntimeError(
                "ViTWarpV8.forward() called without da_feature. "
                "Use WAFT-dav2 (network_backbone='dav2') to load the "
                "DepthAnything feature extractor, or use WAFTv2 with "
                "feature_encoder='twins' for the twins backbone."
            )
        # initial feature
