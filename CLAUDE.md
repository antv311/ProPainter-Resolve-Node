...
- **"Copy Log" button**: sits to the right of "Open results/" in the Tab 1 button row (column 2). Calls `_copy_log()` which does `self.clipboard_clear()` + `self.clipboard_append(self._log_box.get("1.0", tk.END))`. `App` subclasses `tk.Tk` directly so `self` is the root window — no `self.root` indirection needed.
- **`deform_conv2d` arg order (tvdcn 1.1.0)**: both `recurrent_flow_completion.py` (line 45) and `model/propainter.py` `DeformableAlignment.forward()` (line 68) use tvdcn's signature via **keyword arguments**: `deform_conv2d(input=x, weight=self.weight, offset=offset, mask=mask, bias=self.bias, stride=_pair(...), padding=_pair(...), dilation=_pair(...))`. `_pair` in `recurrent_flow_completion.py` is a module-level helper; `propainter.py` imports `_pair` from `torch.nn.modules.utils`. `ModulatedDeformConv2d` stores stride/padding/dilation as bare ints (no `_pair` in `__init__`) so the `_pair()` wrapping at call sites is required.
- **`_tile_inpaint()` (tkinter_app.py)**: mirrors `_tile_flow_complete()` for the `inpaint_model(...)` call. At 4K, `SparseWindowAttention.repeat()` broadcasts pooled global context to 1,296 windows producing ~38.7 GB tensors. Tiling to ≈1080p quarters reduces this to ~1.2 GB FP16. Uses 128px overlap (wider than flow_complete's 64px — transformer attention receptive field spans ~60–108px/window). Triggered by same `h > _FC_TILE_H or w > _FC_TILE_W` guard. Returns `[b*l_t, 3, h, w]` float32 — same shape as the non-tiled path before `(pred + 1) / 2`. Flow vectors NOT adjusted for tile offset.
- **FP16 dtype cast in `_tile_flow()`**: before calling `self._waft(t1, t2)`, each tile is cast via `dtype = next(self._waft.parameters()).dtype` / `t1.to(dtype)`, `t2.to(dtype)`. Result is always cast back `.float()` — downstream flow tensors stay float32 regardless of model dtype.
- **`bilinear_sampler` grid dtype fix (WAFT/utils/utils.py)**: `coords_grid` always returns float32; when FP16 WAFT is on, `img` is float16 and `F.grid_sample` raises a dtype mismatch. Fixed at source: `.to(img.dtype)` appended to the `torch.cat([xgrid, ygrid], ...)` line. Covers all call sites in `waft_a1.py` in one shot.
- **VRAM cleanup at `run_inpainting()` start**: before the `try:` block, `gc.collect()` + `torch.cuda.empty_cache()` + `torch.cuda.reset_peak_memory_stats()` runs on every invocation. Prevents ghost memory from a crashed run (e.g. 26 GB reserved) from polluting the allocator state for subsequent runs in the same session.
