...
- **`deform_conv2d` arg order (tvdcn 1.1.0)**: `recurrent_flow_completion.py` line 42 uses tvdcn's signature `(input, weight, offset, mask, bias, stride, padding, dilation)` — not torchvision's old order. `_pair()` helper defined at module level converts plain `int` stride/padding/dilation to `tuple` as tvdcn requires `List[int]`.
